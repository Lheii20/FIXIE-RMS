    ...
    if(document.getElementById('revenueForecastChart')) {
        fetch(getApiUrl('revenue_forecast')).then(r => r.json()).then(data => {
            ...
            new Chart(ctx, { 
                ...
                options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { position: 'top', align: 'end', labels: { usePointStyle: true, boxWidth: 8, padding: 12 } } }, scales: { x: { grid: { display: false } }, y: { grid: { borderDash: [4, 4], color: '#f1f5f9' }, beginAtZero: true, ticks: { callback: function(value) { return '₱' + value.toLocaleString(); } } } } } 
            });
        });
    }
    ...
    if(document.getElementById('topCategoriesChart')) {
        fetch(getApiUrl('top_categories')).then(r => r.json()).then(data => {
            ...
            new Chart(ctx, { 
                ...
                options: { responsive: true, maintainAspectRatio: false, cutout: '75%', borderRadius: 5, plugins: { legend: { position: 'right', labels: { usePointStyle: true, padding: 12 } }, tooltip: { callbacks: { label: function(context) { return ' ₱' + context.raw.toLocaleString(); } } } } } 
            });
        });
    }
    ...
    if(document.getElementById('workloadChart')) {
        fetch(getApiUrl('workload_distribution')).then(r => r.json()).then(data => {
            ...
            new Chart(ctx, { 
                ...
                options: { responsive: true, maintainAspectRatio: false, cutout: '75%', borderRadius: 4, plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, padding: 12 } } } } 
            });
        });
    }
    ...
    if(document.getElementById('rejectionChart')) {
        fetch(getApiUrl('rejection_rate')).then(r => r.json()).then(data => {
            ...
            new Chart(ctx, { 
                ...
                options: { responsive: true, maintainAspectRatio: false, rotation: -90, circumference: 180, cutout: '80%', borderRadius: 10, plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, padding: 12 } } } } 
            });
        });
    }
    ...
    if(document.getElementById('docRetentionChart')) {
        fetch(getApiUrl('doc_retention_status')).then(r=>r.json()).then(data=>{
            ...
            new Chart(ctx, {
                ...
                options: { responsive: true, maintainAspectRatio: false, cutout: '75%', borderRadius: 5, plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, padding: 12 } } } }
            });
        });
    }
    ...
    if(document.getElementById('adminUserRolesChart')) {
        fetch(getApiUrl('admin_user_roles')).then(r=>r.json()).then(data=>{
            ...
            new Chart(document.getElementById('adminUserRolesChart').getContext('2d'), {
                ...
                options: { responsive: true, maintainAspectRatio: false, cutout: '75%', borderRadius: 4, plugins: { legend: { position: 'right', labels: { usePointStyle: true, padding: 12 } } } }
            });
        });
    }
