...
try {
    $required=[
        'virtual_cabinet.php'=>['cabinet-redesign','data-redesign="vc5e"','assets/css/virtual-cabinet.css','id="vc3LocationPanel"','id="vc3LocationToggle"','class="vc5-custody-select"','id="vc3Custody"','id="vc3LocationSearch"','id="vc3List"','id="vc3Path"','Physical location','includes/physical_record_profile.php','includes/storage_location_manager.php'],
        'assets/css/virtual-cabinet.css'=>['.cabinet-redesign','height:calc(100dvh - 68px)!important','min-height:0!important','margin-top:auto','.vc3-workspace.is-location-collapsed','.vc5-custody-select','@media(max-width:700px)','@media(max-width:480px)','grid-template-columns:minmax(0,1fr) minmax(118px,38%)','#physicalRecordProfile [hidden]','#storageLocationManager [hidden]'],
        'assets/js/physical-cabinet.js'=>["root.dataset.redesign!=='vc5e'",'matchMedia(\'(max-width: 1399px)\')','setLocationPanel','finishLocationChoice',"el('Custody').addEventListener('change'",'dataset.label=\'Location\'','vc5-record','cell.colSpan=4','LocationSearch','syncExport','physical-copy-updated','Content-Disposition','URL.createObjectURL'],
    ];
    foreach($required as $file=>$markers){
        $path=$root.'/'.$file;if(!is_file($path))throw new RuntimeException('Missing file: '.$file);
        $source=file_get_contents($path);foreach($markers as $marker)if(!str_contains($source,$marker))throw new RuntimeException('Incomplete VC5E file: '.$file.' ('.$marker.')');
    }
    ...
    echo "VC5E verification passed. Read-only; no records, files, locations or permissions changed.\n";
