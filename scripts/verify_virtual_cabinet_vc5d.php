...
try {
    $required=[
        'virtual_cabinet.php'=>['cabinet-redesign','data-redesign="vc5d"','assets/css/virtual-cabinet.css','id="vc3LocationPanel"','id="vc3LocationToggle"','class="vc5-custody-tabs"','role="tablist"','id="vc3LocationSearch"','id="vc3List"','id="vc3Path"','Physical location','includes/physical_record_profile.php','includes/storage_location_manager.php'],
        'assets/css/virtual-cabinet.css'=>['.cabinet-redesign','height:calc(100dvh - 68px)!important','min-height:0!important','margin-top:auto','.vc3-workspace.is-location-collapsed','.vc5-custody-tabs','@media(max-width:700px)','grid-template-columns:minmax(0,1fr) minmax(118px,38%)','#physicalRecordProfile [hidden]','#storageLocationManager [hidden]'],
        'assets/js/physical-cabinet.js'=>["root.dataset.redesign!=='vc5d'",'matchMedia(\'(max-width: 1399px)\')','setLocationPanel','finishLocationChoice','syncCustodyTabs','applyCustody','dataset.label=\'Location\'','vc5-record','cell.colSpan=4','LocationSearch','syncExport','physical-copy-updated','Content-Disposition','URL.createObjectURL'],
    ];
    foreach($required as $file=>$markers){
        $path=$root.'/'.$file;if(!is_file($path))throw new RuntimeException('Missing file: '.$file);
        $source=file_get_contents($path);foreach($markers as $marker)if(!str_contains($source,$marker))throw new RuntimeException('Incomplete VC5D file: '.$file.' ('.$marker.')');
    }
    ...
    if(substr_count($page,' role="tab" data-custody=')!==5)throw new RuntimeException('All five visible custody filters must be installed.');
    ...
    echo "VC5D verification passed. Read-only; no records, files, locations or permissions changed.\n";
